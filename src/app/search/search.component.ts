...
export class SearchComponent /*implements OnInit*/ {
  ...
  public initializeNpcTalkMessages() {
    ...
    let analysis = {
      "ConversationWithoutNpcSet": [] as any[],
      "TalkMsgWithoutNpcSet": [] as any[],
      "NpcSetWithoutTalkMsg": [] as any[]
    };
    ...
    for (let id in talkMsgData) {
      ...
      if (talkMsg != null) {
        ...
        if (this.npcTalkMessages[superId] == undefined) {
          ...
          analysis["TalkMsgWithoutNpcSet"].push({"superId": superId, "firstTalkMsgId": id});
          this.npcTalkMessages[superId] = {
    ...
    for (let superId in this.npcTalkMessages) {
      if (Object.keys(this.npcTalkMessages[superId].conversations).length == 0) {
        analysis["NpcSetWithoutTalkMsg"].push({"superId": superId});
      }
    }
    for (let obj of analysis["TalkMsgWithoutNpcSet"]) {
      let superId = obj["superId"];
      console.log("superId=" + superId);
      console.log("orphan conversations:", this.npcTalkMessages[superId]);
    }
    ...
    for (let superId in this.npcTalkMessages) {
      ...
      if (this.anyMatch(text, speakerName)) {
        this.searchResults.push({
          "type": "Character",
          "name": speakerName,
          "info": "Speaker #" + superId,
          "caption": "See dialogue spoken by this character(s)",
